 ...
 * 2012-05-15 -WFC- It only has RF_RemoteKey learning if it has no XBEE.
 * 2012-09-27 -WFC- Call panel_main_update_sleep_mode_display(); skip call panel_main_normal_run_mode() if LED is in sleep state.
 */
...
//				gbPanelMainRunMode = PANEL_RUN_MODE_NORMAL;	//	PHJ ^ was commented out 6/24/10	~~~~
	...
	if ( !( PANEL_RUN_MODE_SELF_TEST == gbPanelMainRunMode ||
		...
		panel_main_update_sleep_mode_display();							// 2012-09-27 -WFC-
		led_display_manager();
	}
 ...
 * 2013-04-26 -WFC- Use high level MACRO SET_RF_LEARN and CLEAR_RF_LEARN instead of low level direct pin toggle.
 */
...
//					PT_INIT( &gPanelSetupTopMenuObj.m_pt );								// init panel setup top menu thread.
			...
			case PANEL_CAL_KEY:
					bios_enabled_comports_in_power_save_state();							// 2014-10-03 -WFC-
					#ifdef  LED_ANC_PEAK
						led_display_set_annunciator( LED_ANC_PEAK, LED_SEG_STATE_OFF);
		...
		case FUNC_KEY_PHOLD:
			...
			if ( SCALE_STD_MODE_INDUSTRY == unit || SCALE_STD_MODE_1UNIT == unit ) { // 2011-09-01 -WFC- Only allows non Legal For Trade mode to perform Peak Hold function.
				...
				if ( pLc-> runModes &  LC_RUN_MODE_PEAK_HOLD_ENABLED )  {
					...
					#ifdef  LED_ANC_PEAK
						led_display_set_annunciator( LED_ANC_PEAK, LED_SEG_STATE_STEADY);
					#endif
				}
...
void panel_main_update_display_data( LOADCELL_T *pLc )
{
	...
	if ( gbIsKeyStuck ) {
		...
		strcpy_P ( gLedDisplayManager.descr.str2, gcStr_buttn);
	}
	// 2011-08-24 -WFC- v
...
//		if ( LC_STATUS_GOT_VALID_WEIGHT & (pLc-> status) || (LC_STATUS3_GOT_PREV_VALID_VALUE & (pLc-> status3)) || // 2011-05-09 -WFC-
			...
			#ifdef  LED_ANC_LOC_STABLE
				led_display_set_annunciator( LED_ANC_LOC_STABLE, ancOffIndex );
			#endif
...
//2012-02-06 -WFC-				}
			...
			if ( LC_STATUS_OVERLOAD & (pLc-> status)  ) {
				...
				gLedDisplayManager.descr.mode = LED_DISPLAY_MODE_ERROR_MSG;		// set to display error message mode
				strcpy_P ( gLedDisplayManager.descr.str1, gcStr_Error);
				strcpy_P ( gLedDisplayManager.descr.str2, gcStr_Load);
			}
				...
				gLedDisplayManager.descr.mode = LED_DISPLAY_MODE_ERROR_MSG;		// set to display error message mode
				strcpy_P ( gLedDisplayManager.descr.str1, gcStr_Error);
				strcpy_P ( gLedDisplayManager.descr.str2, gcStr_UnLd);
			}
			else {
 ...
 * 2011-08-31 -WFC- Called panel_setup_construct_cnfg_menu_object() to handle Legal For Trade mode cnfg menu.
 * 2013-11-08 -WFC- empty key buffer clear previous pressed keys after called panel_main_power_up().
 *
 */
 ...
 * 2012-09-20 -WFC- Moved LED intensity settings codes to led_display_init().
