/*! \file led_display_hw4260B.c \brief Middle layer Level LED display related functions.*/
// ****************************************************************************
//
//                      MSI ScaleCore
//
...
#if ( HW4260B_LED_MAX_DIGIT_LENGTH == 5 ) // 2012-07-12 -DLM-
const char gcStrMaxSpaces[]	PROGMEM = "     ";
#elif ( HW4260B_LED_MAX_DIGIT_LENGTH == 6 )
const char gcStrMaxSpaces[]	PROGMEM = "      ";
#endif

BYTE gbNChDisplayed; // number of char displayed

/**
 * Init led display manager object.
 *
 * History:  Created on 2009/05/28 by Wai Fai Chin
 * 2012-09-20 -WFC- Copied led intensity codes from panel_main_power_up(). 2012-10-03 -DLM-
 ...
 * 2012-09-28 -WFC- 40 timer ticks off if LED is in sleep mode.  2012-10-03 -DLM-
 */
 ...
 * 			 Modified on 2012-06-08 by Denis Monteiro
 */
...
BYTE led_display_format_float_string( float fV, MSI_CB *pCB, BYTE needX1000, BYTE *pOutStr )
{
	...
	if ( needX1000 ) {
//		if ( fV > 99999.4 || fV < -99999.4) { 2012-06-11 -DLM-
		if ( fV > 99999.4 || fV < -9999.4) {
			fV /= 1000.0;
...
*/
	...
	led_display_set_annunciator( LED_DISPLAY_ANC_X1000, LED_SEG_STATE_OFF );
	if ( needX1000 ) {
		// if ( fV > 99999.4 || fV < -99999.4) {	// show decimal point 2012-06-08 -DLM-
		#if ( HW4260B_LED_MAX_DIGIT_LENGTH == 5 )
		if ( fV > 99999.4 || fV < -9999.4) {	// show decimal point
			led_display_set_annunciator( LED_DISPLAY_ANC_X1000, LED_SEG_STATE_STEADY );
			fV /= 1000.0;
			if ( fV > 9999.94 || fV < -999.94 ) {
			// if ( fV > 9999.94 || fV < -9999.94 ) { 2012-06-08 -DLM-
				countby.decPt  = 0;
				countby.fValue = 1.0f;
			}
			else if ( fV > 999.94 || fV < -99.94 ) {
			// else if ( fV > 999.94 || fV < -999.94 ) { 2012-06-08 -DLM-
				countby.decPt  = 1;
				countby.fValue = 0.1f;
			}
			else if ( fV > 99.94 || fV < -9.94 ) {
			// else if ( fV > 99.94 || fV < -99.94 ) { 2012-06-08 -DLM-
				countby.decPt  = 2;
				countby.fValue = 0.01f;
			}
			else if ( fV > 9.94 || fV < -0.94 ) {
			// else if ( fV > 9.94 || fV < -9.94 ) { 2012-06-08 -DLM-
				countby.decPt  = 3;
		...
		#elif ( HW4260B_LED_MAX_DIGIT_LENGTH == 6 )
		if ( fV > 999999.4 || fV < -99999.4) {	// show decimal point
			led_display_set_annunciator( LED_DISPLAY_ANC_X1000, LED_SEG_STATE_STEADY );
			fV /= 1000.0;
			if ( fV > 99999.94 || fV < -9999.94 ) {
			// if ( fV > 9999.94 || fV < -9999.94 ) { 2012-06-08 -DLM-
				countby.decPt  = 0;
				countby.fValue = 1.0f;
			}
			else if ( fV > 9999.94 || fV < -999.94 ) {
			// else if ( fV > 999.94 || fV < -999.94 ) { 2012-06-08 -DLM-
				countby.decPt  = 1;
				countby.fValue = 0.1f;
			}
