...
// ****************************************************************************
// 2013-09-03 -DLM- split from voltmon.c for PortaWeigh2.
...
BYTE gbShutOffCnt;		// 2013-10-02 -WFC- 2013-10-08 -DLM-
BYTE gbBlinkBattCnt;	// 2017-09-12 -DLM-
BYTE gbLowBattCnt;		// 2017-09-12 -DLM-
BYTE gbLowScVoltage;	// 2017-09-12 -DLM-
...
const float gcafVoltmonScaleFactor[ MAX_NUM_VOLTAGE_MONITOR ] PROGMEM = { 0.008069722402, 0.008069722402, 0.0015527349 };


/// offset
// const float gcafVoltmonOffset[ MAX_NUM_VOLTAGE_MONITOR ] PROGMEM = { 17.1, 17.1, 3.18 };
// const float gcafVoltmonOffset[ MAX_NUM_VOLTAGE_MONITOR ] PROGMEM = { 4.275, 4.275, 3.18 };
const float gcafVoltmonOffset[ MAX_NUM_VOLTAGE_MONITOR ] PROGMEM = { 17.074, 17.074, 3.18 };
	...
	if ( (sn <= SENSOR_NUM_LAST_VOLTAGEMON) && ( sn >= SENSOR_NUM_1ST_VOLTAGEMON) )	{
		...
		if ( SENSOR_CNFG_ENABLED & pSnDesc-> cnfg ) {						// if this sensor is enabled.
			if ( SENSOR_STATUS_GOT_ADC_CNT & (pSnDesc-> status) ) {		// if got a new ADC count
				pVm-> status &= ~(VM_STATUS_GOT_VALID_VALUE | VM_STATUS_GOT_CAL_VALUE);	// assumed it has no valid weight
 ...
 * 2013-06-24 -WFC- Modified for ScaleCore3. // 2013-09-03 -DLM-
 */
...
BYTE voltmon_is_under_voltage( BYTE sn )
{
	...
	INT32 offsetAdcCntIS;

	if (gtSystemFeatureFNV.ledIntensity >= 3)
		offsetAdcCntIS = VOLTMON_UNDER_VOLTAGE_HI1_OFFSET_IS * gbNChDisplayed;
	else if (gtSystemFeatureFNV.ledIntensity == 2)
		offsetAdcCntIS = VOLTMON_UNDER_VOLTAGE_LO2_OFFSET_IS * gbNChDisplayed;
	else if (gtSystemFeatureFNV.ledIntensity <= 1)
		offsetAdcCntIS = VOLTMON_UNDER_VOLTAGE_LO1_OFFSET_IS * gbNChDisplayed;


	if ( USER_DEF_MODEL_BATTERY_12V_IS == gtProductInfoFNV.userDefModelCode ) {
		shutOffAdcCnt = VOLTMON_UNDER_VOLTAGE_SHUT_OFF_ADCCNT_12V_IS + offsetAdcCntIS;
		underVBlinkAdcCnt = VOLTMON_UNDER_VOLTAGE_BLINK_ADCCNT_12V_IS + offsetAdcCntIS;
		underVAdcCnt = VOLTMON_UNDER_VOLTAGE_ADCCNT_12V_IS + offsetAdcCntIS;
	}
	else if ( USER_DEF_MODEL_BATTERY_12V == gtProductInfoFNV.userDefModelCode ) {
		shutOffAdcCnt = VOLTMON_UNDER_VOLTAGE_SHUT_OFF_ADCCNT_12V;
		underVBlinkAdcCnt = VOLTMON_UNDER_VOLTAGE_BLINK_ADCCNT_12V;
		underVAdcCnt = VOLTMON_UNDER_VOLTAGE_ADCCNT_12V;
	}
	else if ( USER_DEF_MODEL_BATTERY_6V == gtProductInfoFNV.userDefModelCode ) {
		shutOffAdcCnt = VOLTMON_UNDER_VOLTAGE_SHUT_OFF_ADCCNT_6V;
		underVBlinkAdcCnt = VOLTMON_UNDER_VOLTAGE_BLINK_ADCCNT_6V;
		underVAdcCnt = VOLTMON_UNDER_VOLTAGE_ADCCNT_6V;
	}
	else if ( USER_DEF_MODEL_BATTERY_6D == gtProductInfoFNV.userDefModelCode ) {
		shutOffAdcCnt = VOLTMON_UNDER_VOLTAGE_SHUT_OFF_ADCCNT_6D;
		underVBlinkAdcCnt = VOLTMON_UNDER_VOLTAGE_BLINK_ADCCNT_6D;
		underVAdcCnt = VOLTMON_UNDER_VOLTAGE_ADCCNT_6D;
	}
	else if ( USER_DEF_MODEL_BATTERY_3C == gtProductInfoFNV.userDefModelCode ) {
		shutOffAdcCnt = VOLTMON_UNDER_VOLTAGE_SHUT_OFF_ADCCNT_3C;
		underVBlinkAdcCnt = VOLTMON_UNDER_VOLTAGE_BLINK_ADCCNT_3C;
		underVAdcCnt = VOLTMON_UNDER_VOLTAGE_ADCCNT_3C;
	}
	else {
...
BYTE voltmon_is_under_voltage_checking( BYTE sn, INT32 shutOffAdcCnt, INT32 underVBlinkAdcCnt, INT32 underVAdcCnt )
{
	BYTE					vbat;
	BYTE					status;
	LSENSOR_DESCRIPTOR_T	*pSnDesc[3];		// points to local sensor descriptor
	//VOLTAGEMON_T			*pVm;			// points to a voltage monitor

	status = FALSE;
	if ( sn == SENSOR_NUM_LAST_VOLTAGEMON )	{
		pSnDesc[0] = &gaLSensorDescriptor[ VOLTMON_ANALOG_UNDER_VOLTAGE_SN ];
		pSnDesc[1] = &gaLSensorDescriptor[ VOLTMON_UNDER_VOLTAGE_VBAT1 ];
		pSnDesc[2] = &gaLSensorDescriptor[ VOLTMON_UNDER_VOLTAGE_VBAT2 ];
		//pVm = (VOLTAGEMON_T *) pSnDesc-> pDev;

		if ( USER_DEF_MODEL_AC != gtProductInfoFNV.userDefModelCode ) {			// AC model just verify VF

			// Selects the higher voltage sensor:
			vbat = 1; // Use VBAT1 as default
			if ( ( SENSOR_CNFG_ENABLED & pSnDesc[1]-> cnfg ) && ( SENSOR_CNFG_ENABLED & pSnDesc[2]-> cnfg ) ) {					// if this sensor is enabled.
				if ( ( SENSOR_STATUS_GOT_ADC_CNT & (pSnDesc[1]-> status) ) && ( SENSOR_STATUS_GOT_ADC_CNT & (pSnDesc[2]-> status) ) )  {
					if ( pSnDesc[1]-> curADCcount  <  pSnDesc[2]-> curADCcount )
						vbat = 2;
				}
			}

			if ( SENSOR_CNFG_ENABLED & pSnDesc[vbat]-> cnfg ) {					// if this sensor is enabled.
				if ( SENSOR_STATUS_GOT_ADC_CNT & (pSnDesc[vbat]-> status) ) {		// if got a new ADC count
					status = TRUE;
					if ( pSnDesc[vbat]-> curADCcount < shutOffAdcCnt ) {
					...
					else if ( pSnDesc[vbat]-> curADCcount < underVBlinkAdcCnt ) {
						gbBlinkBattCnt++;
						if ( gbBlinkBattCnt > 20 ) {
							gbBiosSysStatus |= ( BIOS_SYS_STATUS_VOLTAGE_BLINK_WARNING | BIOS_SYS_STATUS_UNDER_VOLTAGE ); // set both blink warning and under voltage flag.
							gbLowBattCnt = 0;
						}
					}
					else if ( pSnDesc[vbat]-> curADCcount < underVAdcCnt ) {
						gbLowBattCnt++;
						if ( gbLowBattCnt > 20 ) {
		...
		} // if ( != USER_DEF_MODEL_AC...) if not AC power supply

		if ( SENSOR_CNFG_ENABLED & pSnDesc[0]-> cnfg ) {					// if this sensor is enabled.
			if ( SENSOR_STATUS_GOT_ADC_CNT & (pSnDesc[0]-> status) ) {		// if got a new ADC count
				if ( pSnDesc[0]-> curADCcount < VOLTMON_UNDER_VOLTAGE_SHUT_OFF_ADCCNT_ANALOG ) {
					gbBiosSysStatus |= ( BIOS_SYS_STATUS_VOLTAGE_BLINK_WARNING | BIOS_SYS_STATUS_UNDER_VOLTAGE ); // set both blink warning and under voltage flag.
					gbLowScVoltage++;
					if ( gbLowBattCnt > 20 ) {
						gbLowScVoltage = 0;
