/*! \file hw3460_key.c \brief external key driver related functions.*/
// ****************************************************************************
//
//                      MSI ScaleCore
//
...
// 
//   This is a key driver on the hardware 3460 PCB.
...
//
// ****************************************************************************
// 2013-04-19 -WFC- ported to ATMEGA192D3.
...
BYTE hw3460_key_scan( BYTE *pbKey );
void hw3460_RF_remote_DCD_read( void );
BYTE hw3460_key_map_hardware_to_vkey( BYTE hw_key );		// -WFC- 2011-03-15.
 ...
 *  
 * History:  Created on 2009-05-26 by Wai Fai Chin
 * 2012-05-15 -WFC- Created a key scan version for XBEE module.
 * 2013-04-26 -WFC- called hw3460_key_detect_pressed() instead of direct reading.
 */
...
//  curKey &= 0X0F;				// in case the compiler use rotate shift carry instruction, I clear the upper nibble.
  curKey = hw3460_key_detect_pressed();				//2013-04-26 -WFC-
...
BYTE hw3460_key_scan( UINT16 *pbKey )
{
 ...
 *  2012-07-02 -WFC- call hw3460_RF_remote_DCD_read() only if Challenger3 is configured as none RF device.
 */

void hw3460_key_read( void )
{
  ...
  if ( hw3460_key_scan( &hw_key ) || bios_is_cal_switch_pressed() )  {		// 2011-04-25 -WFC- if key pad or cal switch pressed, then
	  v_key = hw3460_key_map_hardware_to_vkey( hw_key );
...
BYTE hw3460_key_map_hardware_to_vkey( BYTE hw_key )
{
	BYTE	v_key;	// high level virtual key code
	...
	return v_key;
}// end hw3460_key_map_hardware_to_vkey()
...
BYTE hw3460_key_get( BYTE *pbKey )
{
	UINT8 temptime;
 ...
 * 2013-09-16 -WFC- ported to ScaleCore3 board.
 */

#if ( CONFIG_USE_CPU == CONFIG_USE_ATMEGA1281)

BYTE hw3460_key_detect_pressed( void )
{
	  static BYTE oldKey;
	  BYTE curKey;					// current key
	  BYTE rfKey;					// current key from rf
...
BYTE hw3460_key_detect_pressed( void )
{
	...
	// PORTF is for SPI3 port. pins: excitation XCLK o, XCLKEN o, AUX1,2,3 inputs. AUX1 at PF2 as input.
	PORT_SetDirection( &PORTF, BIOS_EXCITE_XCLK_bm | BIOS_EXCITE_XCLKEN_bm | BIOS_PIN7_bm );			//set XCLK, XCLKEN, and MOSI pins as output

	// key pressed produce a low, inverted it positive logic.
	b = ~(PORT_GetPortValue( &PORTA ));			// read pins inputs from port A which read in PA1=SW4, PA3=SW1, PA4=SW2
	b1 = ~(PORT_GetPortValue( &VPORT1 ));		// b1 = pins inputs from port J.

	// PORTF is for SPI3 port. pins: excitation XCLK o, XCLKEN o, AUX2,3 inputs. AUX1 at PF2 as output.
	PORT_SetDirection( &PORTF, BIOS_EXCITE_XCLK_bm | BIOS_EXCITE_XCLKEN_bm | BIOS_PIN7_bm | BIOS_PIN2_bm );	//set XCLK, XCLKEN, MOSI, and AUX1 PF2 pins as output
	// place PF2 pint output low statement here so it has more time to settle the electrical transition.
	(&PORTF)-> OUT &= ~BIOS_PIN2_bm;		// set PF2 pin output low for scanning SW5 and SW6.

	curKey = 0;
	if ( BIOS_PIN3_bm & b)					// If PA3, SW1 = Power key is pressed.
		curKey |= KEY_POWER_KEY;
	if ( BIOS_PIN4_bm & b)					// If PA4, SW2 = Zero key is pressed.
		curKey |= KEY_ZERO_KEY;
	if ( BIOS_PIN1_bm & b)					// If PA1, SW4 = F1 (USER1) key is pressed.
		curKey |= KEY_USER1_KEY;

	if ( BIOS_PIN1_bm & b1 )				// If PJ1, SW3 = TARE key is pressed.
		curKey |= KEY_TARE_KEY;

	// key pressed produce a low, inverted it postive logic.
	b = ~(PORT_GetPortValue( &PORTA ));		// read pins inputs from port A which read in PA1=SW4, PA3=SW1, PA4=SW2

	// PORTF is for SPI3 port. pins: excitation XCLK o, XCLKEN o, AUX1,2,3 inputs. AUX1 at PF2 as input.
	PORT_SetDirection( &PORTF, BIOS_EXCITE_XCLK_bm | BIOS_EXCITE_XCLKEN_bm | BIOS_PIN7_bm );			//set XCLK, XCLKEN, and MOSI pins as output

	// Since Power and Print keys share the same input pin, prevent them formed a combined keys.
	if ( !(curKey &  KEY_POWER_KEY) )		// Check Print Key only if Power key is NOT pressed
		if ( BIOS_PIN3_bm & b)					// If PA3, SW6 = Print key is pressed.
			curKey |= KEY_PRINT_KEY;

	// Since F1 and F2 keys share the same input pin, prevent them formed a combined keys.
	if ( !(curKey &  KEY_USER1_KEY) )		// Check F2 Key only if F1 key is NOT pressed
		if ( BIOS_PIN1_bm & b)					// If PA1, SW5 = F2 (USER2) key is pressed.
			curKey |= KEY_USER2_KEY;
