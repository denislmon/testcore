/*! \file panel_setup_challenger3.c \brief Implementation of Panel setup menu for Challenger3.*/
// ****************************************************************************
//
//                      MSI ScaleCore
//
...
BYTE	panel_setup_print_string_top_menu_manger(	PANEL_SETUP_TOP_MENU_CLASS *pSetupObj );		// 2012-05-10 -WFC-
...
PANEL_MENU_ITEM_T	gacPanel_Menu_SetPoint[] PROGMEM = {
	...
	{ gcMsg_List_Setpoint_Mode, panel_setup_setpoint_mode_mapper, 0, MENU_SETPOINT_MODE_MAX_ITEMS | PANEL_MENU_ITEM_TYPE_SIMPLE},
 ...
 * 2012-05-16 -WFC- init to no blinking.
 */
			...
			if ( gPanelNumEntryObj.decPtLoc ) {			// if a decimal point had been selected, just toggle x1000 flag.
				...
				if ( gPanelNumEntryObj.strIndex < (LED_DISPLAY_MAX_DIGIT_LENGTH - 1) ) {
					...
					*pCh = '.';
					...
					gPanelNumEntryObj.str[5] = ' ';		//need a pad space for a decimal point number because the decimal point share the same digit. 
				}
				// 2011-05-02 -WFC- v
			...
			if (gPanelNumEntryObj.strIndex ) {
				if ( gLedDisplayManager.descr.digitBlinkPos ) {
					if ( '.' == *pCh ) {
						...
						gPanelNumEntryObj.str[5] = 0;		//null char replaced a pad space char for a none decimal point number because the decimal point share the same digit..
					}
					*pCh = ' ';
 ...
 * 2015-09-24 -WFC- Added FUNC_KEY_PRINT function key.
 */
// moved from panel_setup.h file. 2011-07-22 -WFC- v
...
/////switch between KG and LB or TON and MTON depends on initial state of unit.
...
//const char *gcMsg_List_Func[]			PROGMEM = {gcStr_OFF, gcStr_tESt, gcStr_totAL, gcStr_v_ttL,
//											gcStr_nEtGr, gcStr_Learn, gcStr_Phold, gcStr_Unit, gcStr_Print };	//	-WFC-
//
//const char *gcMsg_List_Func[]			PROGMEM = {gcStr_OFF, gcStr_tESt, gcStr_totAL, gcStr_v_ttL,
//											gcStr_nEtGr, gcStr_tArE, gcStr_Phold, gcStr_2Unit, gcStr_5Unit, gcStr_HIrES, gcStr_Print };
 ...
 * 2012-08-10 -WFC- Change viewing unit instead of showing unit.
 */
...
BYTE	panel_setup_unit_mapper( BYTE actionMode, PANEL_SETUP_SUB_MENU_CLASS *pSetupObj, void *pV )
{
	...
	index = pSetupObj-> selectIndex;
	if ( PANEL_MENU_ACTION_MODE_SET_CNFG_INDEX == actionMode ) {
		// 2012-08-10 -WFC- gabSensorShowCBunitsFNV[ 0 ] = pSetupObj-> selectIndex;					// set new unit for loadcell 0.
		...
		// 2012-08-10 -WFC- index =  gabSensorShowCBunitsFNV[ 0 ];
		index =  gabSensorViewUnitsFNV[ 0 ];  // 2012-08-10 -WFC-
	}
 ...
 *
 * History:  Created on 2009/07/15 by Wai Fai Chin
 * 2012-09-20 -WFC- Rewrote setpoint mode mapper.
...
BYTE	panel_setup_show_rcal_mapper( BYTE actionMode, PANEL_SETUP_SUB_MENU_CLASS *pSetupObj , void *pV )
{
	...
	if ( PANEL_MENU_ACTION_MODE_GET_CNFG_INDEX == actionMode ) {
		pSetupObj-> nextMove	= PSSMC_NEXTMOVE_DISPLAY_FLOAT_EXIT;	// flag it as a special case to exit current thread, so it can re-enter here with PANEL_MENU_ACTION_MODE_EXIT_STEP_1,
	}
	else if ( PANEL_MENU_ACTION_MODE_EXIT_STEP_1 == actionMode ) {
		panel_setup_show_rcal( PANEL_SETUP_LOADCELL_NUM );
		gbPanelSetupStatus = PANEL_SETUP_STATUS_EXIT_SAVE;				// tell parent thread to exit.
...
void	panel_setup_construct_com_port_menu_object( void )
{
	...
	gPanelSetupTopMenuGroupObj.pRootItem		= gacPanel_Menu_Com_Port_Group_Item_Entry;		// points to top menu group.
